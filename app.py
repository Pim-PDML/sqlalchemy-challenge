...
def most_active():
    ...
    most_active_station_stats = session.query(*sel).filter(Station.station == station_num[0][0]).all()
    ...
    most_act_last_12 = session.query(Measurement.date, Measurement.tobs).filter(Measurement.date > year_ago).filter(Measurement.station == station_id).all()
...
def start(start_date):
    ...
    if start_date > recent_date:
        ...
        start_date_results = session.query(*sel).filter(func.strftime("%Y-%m-%d", Measurement.date) >= start_date).group_by(Measurement.date).all()
        dates = []                       
...
def start_end(start_date, end_date):
    ...
    if start_date > recent_date or start_date > end_date :
        ...
        start_end_results = session.query(*sel).filter(func.strftime("%Y-%m-%d", Measurement.date) >= start_date)\
